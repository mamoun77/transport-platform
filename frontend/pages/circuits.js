...
export default function Circuits() {
  ...
  const handleBooking = (e) => {
    ...
    localStorage.setItem('bookingData', JSON.stringify({
      ...
      price,
      phone: form.phone, email: form.email, name: form.name,
    }));
    setSubmitting(false);
    setSelected(null);
    ...
    <>
      ...
      <div className="min-h-screen bg-[#080d1a] text-white">
        ...
        <section className="relative overflow-hidden pt-32 pb-20 px-6">
          <div className="absolute inset-0 bg-gradient-to-br from-amber-900/30 via-transparent to-orange-900/20 pointer-events-none" />
          <div className="absolute top-20 left-1/4 w-96 h-96 bg-amber-600/10 rounded-full blur-3xl pointer-events-none" />
          <div className="relative max-w-3xl mx-auto text-center">
            <span className="inline-block px-4 py-1.5 mb-6 text-xs font-semibold tracking-widest uppercase rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-400">
              {t('pages:circuits.badge')}
            </span>
            <h1 className="text-5xl sm:text-6xl font-extrabold leading-tight mb-6 bg-gradient-to-r from-white via-amber-100 to-amber-400 bg-clip-text text-transparent">
              {t('pages:circuits.title')}<br />{t('pages:circuits.title2')}
            </h1>
            <p className="text-lg text-slate-400 max-w-xl mx-auto">
              {t('pages:circuits.subtitle')}
            </p>
          </div>
        </section>
              ...
              {circuits.map(c => (
                <div key={c.id} className="group relative flex flex-col rounded-3xl overflow-hidden border border-white/5 bg-white/[0.03] hover:bg-white/[0.07] transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-amber-900/20 h-full">
                  ...
                  <div className="relative h-52 overflow-hidden">
                    ...
                    {c.difficulty && (
                      <span className={`absolute top-4 left-4 px-3 py-1 rounded-full text-xs font-bold border ${DIFFICULTY_COLORS[c.difficulty] || DIFFICULTY_COLORS.facile}`}>
                        {c.difficulty}
                      </span>
                    )}
                    {c.price > 0 && (
                  ...
                  <div className="flex flex-col flex-1 p-6 gap-4">
                    <div className="flex-1">
                      <h3 className="text-xl font-bold text-white mb-2 group-hover:text-amber-300 transition-colors">{c.name}</h3>
                      <p className="text-sm text-slate-400 line-clamp-2 leading-relaxed">{c.short_description || c.description}</p>
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-white/5">
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => setDetail(c)}
                          ...
                          {t('pages:booking.details')}
                        </button>
                        <button onClick={() => setSelected(c)}
                          className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold bg-gradient-to-r ${c.gradient} shadow-lg hover:scale-105 transition-transform`}>
            ...
            <div className="p-8">
                  ...
                  <div className="space-y-2">
                    {detail.program.map((step, i) => (
                      <div key={i} className="flex gap-3 text-sm text-slate-400">
                        <span className="w-6 h-6 rounded-full bg-amber-500/20 border border-amber-500/40 text-amber-400 flex items-center justify-center text-xs flex-shrink-0">{i + 1}</span>
                        {step}
                      </div>
                    ))}
                  </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                {detail.included?.length > 0 && (
                  <div>
                    <h3 className="font-bold text-white mb-2 text-sm">{t('pages:detail_modal.included')}</h3>
                    <ul className="space-y-1">{detail.included.map((x, i) => <li key={i} className="text-xs text-slate-400">• {x}</li>)}</ul>
                  </div>
                )}
                {detail.not_included?.length > 0 && (
                  <div>
                    <h3 className="font-bold text-white mb-2 text-sm">{t('pages:detail_modal.not_included')}</h3>
                    <ul className="space-y-1">{detail.not_included.map((x, i) => <li key={i} className="text-xs text-slate-400">• {x}</li>)}</ul>
                  </div>
                )}
              </div>
              ...
              <button onClick={() => { setDetail(null); setSelected(detail); }}
                className={`w-full py-3 rounded-2xl font-bold text-white bg-gradient-to-r ${detail.gradient || 'from-amber-500 to-orange-600'} hover:scale-[1.02] transition-transform`}>
                {t('pages:circuits.reserve_btn')}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-[#0f172a] border border-white/10 rounded-3xl p-8 w-full max-w-md shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <div>
                ...
                <p className="text-amber-400 text-sm">{selected.name}</p>
              </div>
            ...
            <form onSubmit={handleBooking} className="space-y-4">
              <input type="text" placeholder={t('pages:booking.full_name')} required value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm placeholder-slate-600 focus:outline-none focus:border-amber-500 transition" />
              <input type="tel" placeholder={t('pages:booking.phone')} required value={form.phone} onChange={e => setForm(p => ({ ...p, phone: e.target.value }))}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm placeholder-slate-600 focus:outline-none focus:border-amber-500 transition" />
              <input type="email" placeholder={t('pages:booking.email')} value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm placeholder-slate-600 focus:outline-none focus:border-amber-500 transition" />
              <div className="grid grid-cols-2 gap-3">
                <input type="date" required min={new Date().toISOString().split('T')[0]} value={form.date} onChange={e => setForm(p => ({ ...p, date: e.target.value }))}
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-amber-500 transition" />
              ...
              <button type="submit" disabled={submitting}
                className={`w-full py-3 rounded-2xl font-bold text-white bg-gradient-to-r ${selected.gradient || 'from-amber-500 to-orange-600'} hover:scale-[1.02] transition-transform disabled:opacity-50`}>
