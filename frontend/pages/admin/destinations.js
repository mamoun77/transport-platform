...
export default function AdminDestinations() {
  ...
  const handleEdit = (destination) => {
    ...
    setFormData({
      ...
      distance_from_city: destination.distance_from_city || '',
  ...
  const addAttraction = () => {
    setFormData(prev => ({ ...prev, attractions: [...prev.attractions, ''] }));
  };
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {showForm && (
          <div className="bg-white rounded-lg shadow-md p-6 mb-8">
            <h2 className="text-xl font-semibold mb-4">{editingDestination ? 'Modifier la destination' : 'Nouvelle destination'}</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <input type="text" name="name" value={formData.name} onChange={handleInputChange} placeholder="Nom de la destination *" required className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
              ...
              <MultiImageUpload
                ...
                uploadEndpoint="destination-images"
              />
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <input type="number" name="price" value={formData.price} onChange={handleInputChange} placeholder="Prix (MAD)" min="0" step="0.01" className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              ...
              <div>
                ...
                {formData.attractions.map((attraction, index) => (
                  <div key={index} className="flex gap-2 mb-2">
                    <input type="text" value={attraction} onChange={(e) => handleAttractionChange(index, e.target.value)} placeholder="Attraction" className="flex-1 border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    <button type="button" onClick={() => removeAttraction(index)} className="bg-red-500 text-white px-3 py-2 rounded hover:bg-red-600">×</button>
                  </div>
                ))}
                <button type="button" onClick={addAttraction} className="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600">+ Ajouter</button>
              </div>
