...
export default function Activites() {
  ...
  return (
    <>
      <Head><title>Activités — Trendy Travel</title></Head>
      <div className="min-h-screen bg-[#080d1a] text-white">
        ...
        <section className="relative overflow-hidden pt-32 pb-20 px-6">
          ...
          <div className="relative max-w-3xl mx-auto text-center">
            <span className="inline-block px-4 py-1.5 mb-6 text-xs font-semibold tracking-widest uppercase rounded-full bg-violet-500/10 border border-violet-500/30 text-violet-400">
              {t('pages:activites.badge')}
            </span>
            <h1 className="text-5xl sm:text-6xl font-extrabold leading-tight mb-6 bg-gradient-to-r from-white via-violet-100 to-violet-400 bg-clip-text text-transparent">
        ...
        <main className="max-w-7xl mx-auto px-6 pb-24">
          {activities.length === 0 ? (
            <div className="text-center py-20 text-slate-400">
              <p className="text-xl mb-2">{t('pages:activites.empty')}</p>
              <p className="text-sm">{t('pages:activites.empty_sub')}</p>
            </div>
          ) : (
              ...
              {activities.map(a => (
                <div key={a.id} className="group relative flex flex-col rounded-3xl overflow-hidden border border-white/5 bg-white/[0.03] hover:bg-white/[0.07] transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-violet-900/20 h-full">
                  <div className="relative h-52 overflow-hidden">
                    ...
                    {a.price > 0 && <span className="absolute top-4 right-4 px-3 py-1 rounded-full text-xs font-bold bg-black/50 border border-white/10 backdrop-blur-sm">{format(a.price)}</span>}
                    {a.is_featured && <span className="absolute bottom-4 left-4 px-2 py-0.5 rounded-full text-xs font-bold bg-yellow-400/20 border border-yellow-400/40 text-yellow-300">⭐ {t('pages:booking.vedette')}</span>}
                    {(a.images && a.images.length > 1) && <span className="absolute bottom-4 right-4 px-2 py-0.5 rounded-full text-xs font-semibold bg-black/60 text-white backdrop-blur-sm">📷 {a.images.length}</span>}
                  </div>
                  <div className="flex flex-col flex-1 p-6 gap-4">
                    <div className="flex-1">
                      <h3 className="text-xl font-bold text-white mb-2 group-hover:text-violet-300 transition-colors">{a.name}</h3>
                      <p className="text-sm text-slate-400 line-clamp-2 leading-relaxed">{a.short_description || a.description}</p>
                      <div className="flex flex-wrap gap-3 mt-3">
                        {a.duration && <span className="flex items-center gap-1 text-xs text-slate-400"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6l4 2"/></svg>{a.duration}</span>}
                        {a.location && <span className="flex items-center gap-1 text-xs text-slate-400">📍 {a.location}</span>}
                      </div>
                    </div>
                    <div className="flex items-center justify-between pt-4 border-t border-white/5">
                      <div>
                        <div className="text-lg font-extrabold text-white">
                          {a.price > 0 ? <span>{format(a.price)} <span className="text-xs font-normal text-slate-400">{t('common:common.per_person')}</span></span> : <span className="text-sm text-slate-400">{t('common:common.on_request')}</span>}
                        </div>
                        {a.price_luxury > 0 && <div className="text-sm font-bold text-yellow-400">{format(a.price_luxury)} <span className="text-xs font-normal text-yellow-600">{t('pages:booking.luxe')}</span></div>}
                      </div>
                      <div className="flex gap-2">
                        <button onClick={() => setDetail(a)} className="px-3 py-2 rounded-xl text-xs font-semibold border border-white/10 text-slate-300 hover:bg-white/8 transition">{t('pages:booking.details')}</button>
                        <button onClick={() => setSelected(a)} className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold bg-gradient-to-r ${a.gradient} shadow-lg hover:scale-105 transition-transform`}>
          ...
          <div className="bg-[#0f172a] border border-white/10 rounded-3xl w-full max-w-2xl shadow-2xl my-auto">
            ...
            <div className="p-8">
                ...
                <div className="mb-5">
                  ...
                  <div className="space-y-2">{detail.program.map((step, i) => (
                    <div key={i} className="flex gap-3 text-sm text-slate-400">
                      <span className="w-6 h-6 rounded-full bg-violet-500/20 border border-violet-500/40 text-violet-400 flex items-center justify-center text-xs flex-shrink-0">{i + 1}</span>{step}
                    </div>
                  ))}</div>
                </div>
              )}
              ...
              <button onClick={() => { setDetail(null); setSelected(detail); }}
                className={`w-full py-3 rounded-2xl font-bold text-white bg-gradient-to-r ${detail.gradient || 'from-violet-500 to-purple-600'} hover:scale-[1.02] transition-transform`}>
                {t('pages:activites.reserve_btn')}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-[#0f172a] border border-white/10 rounded-3xl p-8 w-full max-w-md shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <div>
                ...
                <p className="text-violet-400 text-sm">{selected.name}</p>
              </div>
            ...
            <form onSubmit={handleBooking} className="space-y-4">
              <input type="text" placeholder={t('pages:booking.full_name')} required value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm placeholder-slate-600 focus:outline-none focus:border-violet-500 transition" />
              <input type="tel" placeholder={t('pages:booking.phone')} required value={form.phone} onChange={e => setForm(p => ({ ...p, phone: e.target.value }))} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm placeholder-slate-600 focus:outline-none focus:border-violet-500 transition" />
              <input type="email" placeholder={t('pages:booking.email')} value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm placeholder-slate-600 focus:outline-none focus:border-violet-500 transition" />
              <div className="grid grid-cols-2 gap-3">
                <input type="date" required min={new Date().toISOString().split('T')[0]} value={form.date} onChange={e => setForm(p => ({ ...p, date: e.target.value }))} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-violet-500 transition" />
                <input type="time" required value={form.time} onChange={e => setForm(p => ({ ...p, time: e.target.value }))} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-violet-500 transition" />
              </div>
              <div className="flex items-center gap-3">
                ...
                {selected.price > 0 && <span className="ml-auto text-slate-400 text-sm">{t('pages:booking.total')} <span className="text-white font-bold">{format((form.type === 'luxury' && selected.price_luxury > 0 ? selected.price_luxury : selected.price) * form.passengers)}</span></span>}
              </div>
              <textarea placeholder={t('pages:booking.notes')} rows={3} value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm placeholder-slate-600 focus:outline-none focus:border-violet-500 transition resize-none" />
              <button type="submit" disabled={submitting} className={`w-full py-3 rounded-2xl font-bold text-white bg-gradient-to-r ${selected.gradient || 'from-violet-500 to-purple-600'} hover:scale-[1.02] transition-transform disabled:opacity-50`}>
