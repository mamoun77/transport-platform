...
export default function Services() {
  ...
  return (
    <>
      <Head><title>Services — Trendy Travel</title></Head>
        ...
        <section className="relative overflow-hidden pt-32 pb-20 px-6">
          ...
          <div className="relative max-w-3xl mx-auto text-center">
            ...
            <h1 className="text-5xl sm:text-6xl font-extrabold leading-tight mb-6 bg-gradient-to-r from-white via-blue-100 to-blue-400 bg-clip-text text-transparent">
              Transport Premium<br />à votre mesure
            </h1>
            <p className="text-lg text-slate-400 max-w-xl mx-auto">
              Des expériences de voyage soigneusement conçues pour chaque besoin — confort, style et ponctualité garantis.
            </p>
                ...
                return (
                  <div key={s.id} className="group relative flex flex-col rounded-3xl overflow-hidden border border-white/5 bg-white/[0.03] hover:bg-white/[0.07] transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-blue-900/20 h-full">
                    ...
                    <div className="relative h-52 overflow-hidden">
                      <img src={s.image || 'https://images.unsplash.com/photo-1544620347-c4fd4a3d5957?auto=format&fit=crop&w=800&q=80'}
                      ...
                      {s.type && (
                        <span className={`absolute top-4 left-4 px-3 py-1 rounded-full text-xs font-bold border ${cfg.color}`}>
                          {cfg.label}
                        </span>
                      )}
                      ...
                      <div className="flex items-center justify-between pt-4 border-t border-white/5">
                        <div>
                          <div className="text-lg font-extrabold text-white">
                            {(s.price_from || s.price) > 0
                              ? <span>{format(s.price_from || s.price)} <span className="text-xs font-normal text-slate-400">/ pers.</span></span>
                              : <span className="text-sm text-slate-400">Prix sur demande</span>}
                          </div>
                          {s.price_luxury > 0 && (
                            <div className="text-sm font-bold text-yellow-400">
                              {format(s.price_luxury)} <span className="text-xs font-normal text-yellow-600">luxe ✨</span>
                            </div>
                          )}
                        </div>
                        <div className="flex gap-2">
                          ...
                          <button onClick={() => setSelected(s)}
                            className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold bg-gradient-to-r ${s.gradient} shadow-lg hover:scale-105 transition-transform`}>
                            Réserver
      ...
      {detail && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-4 py-8 overflow-y-auto">
          <div className="bg-[#0f172a] border border-white/10 rounded-3xl w-full max-w-2xl shadow-2xl my-auto">
            {detail.image && <img src={detail.image} alt={detail.name} className="w-full h-52 object-cover rounded-t-3xl" style={{ filter: 'brightness(0.7)' }} />}
            <div className="p-8">
                  ...
                  {(detail.price_from || detail.price) > 0 && (
                    <div className="flex gap-3 flex-wrap">
                      <span className="px-3 py-1 rounded-full text-sm bg-white/5 border border-white/10 text-white font-bold">
                        Standard : {format(detail.price_from || detail.price)} / pers.
                      </span>
              ...
              {detail.program?.length > 0 && (
                <div className="mb-5">
                  <h3 className="font-bold text-white mb-3">Programme</h3>
                  <div className="space-y-2">
                    {detail.program.map((step, i) => (
                      <div key={i} className="flex gap-3 text-sm text-slate-400">
                        <span className="w-6 h-6 rounded-full bg-blue-500/20 border border-blue-500/40 text-blue-400 flex items-center justify-center text-xs flex-shrink-0">{i + 1}</span>
                        {step}
                      </div>
                    ))}
                  </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                {detail.included?.length > 0 && (
                  <div>
                    <h3 className="font-bold text-white mb-2 text-sm">✅ Inclus</h3>
                ...
                {detail.not_included?.length > 0 && (
                  <div>
                    <h3 className="font-bold text-white mb-2 text-sm">❌ Non inclus</h3>
              ...
              {detail.luxury_advantages?.length > 0 && (
                <div className="mb-6 p-4 rounded-2xl bg-yellow-500/10 border border-yellow-500/20">
                  <h3 className="font-bold text-yellow-400 mb-3 text-sm">✨ Avantages formule Luxe</h3>
          ...
          <div className="bg-[#0f172a] border border-white/10 rounded-3xl p-8 w-full max-w-md shadow-2xl">
            ...
            {!selected.price_luxury && (
              <div className="mb-5 p-4 rounded-2xl bg-white/5 border border-white/10 flex justify-between items-center">
                ...
                <span className="text-2xl font-extrabold text-white">{(selected.price_from || selected.price) > 0 ? format(selected.price_from || selected.price) : 'Sur demande'}</span>
              </div>
            )}

            <form onSubmit={handleBooking} className="space-y-4">
              <input type="text" placeholder="Votre nom complet *" required value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))}
              ...
              <input type="tel" placeholder="Téléphone *" required value={form.phone} onChange={e => setForm(p => ({ ...p, phone: e.target.value }))}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm placeholder-slate-600 focus:outline-none focus:border-blue-500 transition" />
              ...
              <div className="flex items-center gap-3">
                <label className="text-slate-400 text-sm">Passagers :</label>
              ...
              <input type="text" placeholder="Numéro de vol (ex: AT204)" value={form.flight_number} onChange={e => setForm(p => ({ ...p, flight_number: e.target.value }))}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm placeholder-slate-600 focus:outline-none focus:border-blue-500 transition" />
              <textarea placeholder="Remarques / demandes spécifiques..." rows={3} value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))}
              ...
              <button type="submit" disabled={submitting}
                className={`w-full py-3 rounded-2xl font-bold text-white bg-gradient-to-r ${selected.gradient || 'from-sky-500 to-blue-600'} hover:scale-[1.02] transition-transform disabled:opacity-50`}>
