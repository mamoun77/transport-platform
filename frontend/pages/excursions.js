                ...
                <div>
                  ...
                  {detail.price > 0 && (
                    <div className="flex gap-3 flex-wrap mt-1">
                      <span className="px-3 py-1 rounded-full text-sm bg-white/5 border border-white/10 text-white font-bold">{t('pages:booking.standard')} : {format(detail.price)}</span>
                      {detail.price_luxury > 0 && <span className="px-3 py-1 rounded-full text-sm bg-yellow-500/10 border border-yellow-500/30 text-yellow-400 font-bold">✨ {t('pages:booking.luxury')} : {format(detail.price_luxury)}</span>}
                    </div>
                  )}
                </div>
            ...
            <form onSubmit={handleBooking} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="relative">
                  <input type="date" required min={new Date().toISOString().split('T')[0]} value={form.date} onChange={e => setForm(p => ({ ...p, date: e.target.value }))} className="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3 pr-12 text-white text-sm placeholder-slate-500 focus:outline-none focus:border-emerald-500 transition" />
                ...
                <div className="relative">
                  <input type="time" required value={form.time} onChange={e => setForm(p => ({ ...p, time: e.target.value }))} className="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3 pr-12 text-white text-sm placeholder-slate-500 focus:outline-none focus:border-emerald-500 transition" />
                  <span className="pointer-events-none absolute inset-y-0 right-3 flex items-center text-slate-400">
              ...
              {selected.price > 0 && (() => {
                ...
                const supp = displaySupplement(base);
