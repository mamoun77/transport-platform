...
export default function ServiceDetail() {
  ...
  const calculatePrice = async (routeId, passengers) => {
    ...
    try {
      const r = await axios.post('/backend/routes/calculate-price', { routeId, passengers: parseInt(passengers) });
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const price = calcPrice(service?.price_from || 0, formData.passengers);
                  ...
                  <div className="p-6 bg-gradient-to-r from-blue-600/20 to-indigo-600/20 border-b border-white/8">
                    <div className="flex items-end gap-2 mb-1">
                      <span className="text-4xl font-extrabold text-white">
                        {service.price_from ? format(calcPrice(service.price_from, formData.passengers)) : '—'}
                      </span>
                    </div>
